...
int i,j,a,b,st1,st2,it1,it2,e,t;
FILE *pf1;
			...
			if(st1 != st2) // Si les P-states sont different, sinon on est à la diagonale 
				{
					...
					for( it1 = 0; it1 <= a + b ; it1++ )
					{
						it2 = a + b - it1;
						
						if( it1 <= BufferSize && it2 <= BufferSize ) 
						{
							/*id1 = Id_Etat(it1);
							id2 = Id_Etat(it2);*/
							EngTmp = Eng_Etat(it1, it2, (int)fabs(a - it1), EB);
				...
				/*if(NMigrations[i] != 0)
				{
					printf("e = %d  states (%d , %d) : %.Lf  ---> (%d  , %d) : %.Lf et %d Migrations \n",e,TabInit[i].state1,TabInit[i].state2, EngInit[i], TabOpt[i].state1, TabOpt[i].state2, EngOpt[i], NMigrations[i]);
					fprintf(pf1,"%5d %5d %5d %5d %5d %5d\n",TabInit[i].state1,TabInit[i].state2,e,NMigrations[i],TabOpt[i].state1,TabOpt[i].state2);
				}
				else
				{
					fprintf(pf1,"%5d %5d %5d    NaN   NaN  NaN \n",TabInit[i].state1,TabInit[i].state2,e);
				}*/
		}
