	...
	for(i=0; i<taille*NEt ; i=i+NEt){
		...
		if(gamma12 != 0 )
		{
			for( j=0; j<tailleM; j++)
			{
				...
				{						
					Cons1 += eb*NMigrations[j]*pi[(int)i/NEt];   // Old
	...
	for(i=0; i<taille*NEt ; i=i+NEt){
		...
		if(gamma12 != 0 )
		{
			for( j=0; j<tailleM; j++)
			{
				...
				{						
					Cons2 += eb*NMigrations[j]*pi[(int)i/NEt];   // Old
		...
		if(gamma12 != 0 )
		{
			for( j=0; j<tailleM; j++)
			{
				if( SMigrations[j] == 1 && Etats1[j] == tab[i] &&  Etats2[j] == tab[i+1] )
				{
					...
					ConsMoy1 += NMigrations[j]*eb*pi[(int)i/NEt];					     // New equation
					break;
				}
			}
		}
		...
		if(gamma12 != 0 )
		{
			for( j=0; j<tailleM; j++)
			{
				if( SMigrations[j] == 2 && Etats1[j] == tab[i] &&  Etats2[j] == tab[i+1] )
				{
					...
					//ConsMoy1 += /*gamma12*/eb*NMigrations[j]*pi[(int)i/NEt];  // Old equation
					ConsMoy1 += eb*NMigrations[j]*pi[(int)i/NEt];		// New equation
					break;
				}
