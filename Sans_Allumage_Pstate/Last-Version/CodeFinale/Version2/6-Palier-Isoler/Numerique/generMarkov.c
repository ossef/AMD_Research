...
void  usage()
{
	printf("usage : GenerMatrix -f filename Lambda1 seuil1 seuil2 seuil3 seuil4 seuil5\n");
...
char *argv[];
{
	Etat F, G, R;
	item It, It2;
	int j, k, bcli,seuil1,seuil2,seuil3 ,seuil4, seuil5, Lambda1;
	double SommeDeProba;
	short deja;
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(It.voisin);
#endif
			Equation(It.voisin, j, F, R, seuil1, seuil2, seuil3, seuil4,seuil5);
#if (DEBUG==1)
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin, Lambda1, seuil1, seuil2, seuil3, seuil4,seuil5);
			SommeDeProba+=pro;
#if (DEBUG==1)
			printf("Proba : %lf\n", pro);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin, Lambda1, seuil1,seuil2, seuil3, seuil4, seuil5)/2;
			SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
        printf("SommeDeProba : %lf\n", SommeDeProba);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro1 = Probabilite(j, It.voisin, Lambda1, seuil1,seuil2, seuil3 , seuil4, seuil5);
			for (k = 1; k <= NbEvtsPossibles; k++) {
			Equation(It.voisin, j, F, R, seuil1, seuil2, seuil3, seuil4,seuil5);
#if (DEBUG==2)
				PrintEtat(G);
#endif
				pro2 = Probabilite(k, F, Lambda1, seuil1,seuil2, seuil3 , seuil4, seuil5);
				pro = pro1 * pro2 /2.0; 
				SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
